...
# 1. Обнаруживаются старые переменные окружения
# 2. Создается резервная копия: /opt/remnanode/.env.backup.TIMESTAMP
# 3. Миграция APP_PORT → NODE_PORT
# 4. Миграция SSL_CERT → SECRET_KEY
# 5. Перезапуск контейнера с новой конфигурацией
```
